...
--- get the total of rides 4740357 (from April to Nov 2017)
select count(*) from tbl_stationtrips;


--- get the top station by percentage
SELECT 
name,
(COUNT(*) / 4740357 ) * 100 as percentage
FROM tbl_stations st INNER JOIN tbl_stationtrips trp
ON st.code = trp.start_station_code
GROUP BY name
ORDER BY 2 DESC
...
---- counting number of trips per station
